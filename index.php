...
function loadAllHistories(id_ppic_pi) {
    const historyTypes = ['kd_hb', 'kd_hb_selesai', 'pembahanan', 'pembahanan_selesai', 'finishing',
                            'finishing_selesai','fitting','fitting_selesai','packing','packing_selesai'];
...
function showNoHistoryMessage() {
    const containers = [
        'kd_hb_history', 'kd_hb_selesai_history',
        'pembahanan_history', 'pembahanan_selesai_history', 'finishing_history',
        'finishing_selesai_history','fitting_history','fitting_selesai_history','packing_history','packing_selesai_history'];
...
$('#myModal').on('hidden.bs.modal', function () {
    // Reset form fields except PI selection
    var currentPI = $('#pilih_pi').val();
    $('#form-ppic-pi')[0].reset();
    $('#pilih_pi').val(currentPI).trigger('change');
    
    // Reset other selects
    $('#section').val('').trigger('change');
});

// Add a variable to store the PI selection globally
let lastSelectedPI = '';

// Modify the modal show handler to restore the last selection
    ...
    $.ajax({
        ...
        success:function(result){
            ...
            for(var i=0;i<result.length;i++){
                opt+='<option value="'+result[i].id_dpi+'">'+result[i].NoPI+' | '+result[i].POBuyer+' | '+result[i].KD_ITMBYR+' | '+result[i].DESKRIPTION+'</option>';
            }
            $('#pilih_pi').html(opt);
            
            // Restore last selection if it exists
            if(lastSelectedPI) {
                $('#pilih_pi').val(lastSelectedPI).trigger('change');
            }
        }
    });
});

// Store the selection when it changes
...
$('#form-ppic-pi').submit(function(e){
    ...
    $.ajax({
        ...
        success:function(data){
            ...
            // $('#section').val('').trigger('change');
            $('#preparation_type').val('').trigger('change');
            $('#purchasing_type').val('').trigger('change');
            $('#production_type').val('').trigger('change');
            $('#warehouse_type').val('').trigger('change');
